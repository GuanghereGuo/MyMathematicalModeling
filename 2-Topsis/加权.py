...
flag = (
    True if input("是否需要给指标加权？(y/N):") == "y" else False
)  # 提示用户是否需要加权

weight = [1] * m  # 初始化权重向量，默认每个指标的权重为1
if flag:  # 如果需要加权
    print("请输入权重向量：")
    weight = input().split(" ")  # 接收权重向量
    print(f"权重向量为：\n{weight}")  # 打印权重向量
    weight = list(map(float, weight))  # 将权重向量转换为浮点数列表
...
# 计算加权矩阵
for j in range(m):
    X[:, j] *= weight[j]  # 将标准化后的矩阵X与权重向量相乘，得到加权矩阵W
print(f"加权矩阵为：\n{X}")
...
d_z = np.sqrt(
    np.sum(np.square((X - np.tile(x_max, (n, 1)))), axis=1)
)  # 计算每个参评对象与最优情况的距离d+
d_f = np.sqrt(
    np.sum(np.square((X - np.tile(x_min, (n, 1)))), axis=1)
)  # 计算每个参评对象与最劣情况的距离d-
